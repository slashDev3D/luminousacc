...
$footer_arr = explode("\n", $buffer);
    foreach ($footer_arr as $footer_line) {
        ...
        if (strpos($footer_line,'/wp-content/plugins/wp3d-models/assets/') == true || strpos($footer_line,'maps.googleapis.com') == true) {
...
$skin_path = plugins_url().'/wp3d-models/skins/crosby';
...
<head>
    ...
    $this_skin_path = 'plugins/wp3d-models/skins/crosby/'; // trailingslashed
    ...
    <div id="page-wrap">
        ...
        <div class="wp3d-embed-wrap<?php echo $status_class; ?>" itemscope itemtype="http://schema.org/<?php echo esc_attr(apply_filters( 'wp3d_content_schema_type', $content_schema_type )); ?>">
            ...
            <?php if (!$is_static_image) { // no need to do all of this iframe genration for a static image ?>
                ...
                <iframe id="mp-iframe" src="" <?php if ($wp3d_preload) { echo 'data-preload="true" '; } ?>data-src="<?php echo $wp3d_iframe_data_src; ?>" frameborder="0" allow="vr" allowfullscreen></iframe>
...
if(get_option('wp3d_disclaimer_text') || 'on' != get_option('wp3d_hide_branding')) { ?>    
<section id="disclaimer" class="clearfix">
    ...
    <?php if ('on' != get_option('wp3d_hide_branding') ) { // checking for option to hide the WP3D Models branding" 
