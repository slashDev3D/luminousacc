...
function wp3d_deactivate_license() {
	...
	function getLicenseData($license_key, $action) {
        ...
        if (is_multisite()) {
            $item_name = WP3D_MODELS_MULTISITE_PLUGIN_NAME;
        }
    ...
    public function getRenewLink() {
        $item_id = WP3D_MODELS_SINGLE_LICENSE_ID;
        if (is_multisite()) {
            $item_id = WP3D_MODELS_MULTI_LICENSE_ID;
        }
    ...
    public function expire_notice() {
        ...
        if (in_array($post_type, ['model', 'wp3d_agent'])) {
            ...
            $license_status = trim(get_option('wp3d_license_status'));
            if ($license_status != 'valid') {
                echo '<div class="error notice">
                        <p>Your license is invalid. Please buy a valid <a target="_blank" href="https://wp3dmodels.com/buy-now/">WP3D Models license</a>.</p>
                       </div>';

                return;
            }

            $expire_date = $this->getLicenseExpireDate();
            if ($expire_date <= time()) {
                //show expired notice
                echo '<div class="error notice">
                            <p>You are using an expired version of WP3D Models that is no longer supported.  You will no longer receive important patches / fixes to the plugin, support assistance, or access to current/future versions until you renew. <a target="_blank" href="' . $this->getRenewLink() . '">Renew today for 30% off (offer valid for a limited time)</a>.</p>
                           </div>';
            } elseif ($expire_date - time() < 60 * 60 * 24 * 30) {
                //show almost expired notice
                $days = ceil(($expire_date - time()) / (60 * 60 * 24));
                echo '<div class="notice-warning notice">
                            <p>WP3D Models will expire in ' . $days . ' days, renew now for continued Support, Versions, Bug Fixes, and Feature Enhancements.  Renew today for <a target="_blank" href="' . $this->getRenewLink() . '">30% off</a>!</p>
                          </div>';
            }
        }
    }
